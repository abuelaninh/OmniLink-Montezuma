...
function bfs(grid: string[], start: Position, targetChar: string, keyCollected: boolean): ("UP" | "DOWN" | "LEFT" | "RIGHT")[] {
    ...
    while (queue.length > 0) {
        ...
        for (const dir of dirs) {
            ...
            // Wall / Spike / Gate check
            if (cell === '#' || cell === 'S' || cell === 'G') continue;
    ...
    try {
        ...
        if (wrapper.command === "ACTIVATE") {
            ...
            if (!currentSwitchState && lastSwitchState) {
                console.log("[AGENT] Switch Activated! Repathing to Key...");
                lastSwitchState = false;
                currentPath = []; // reset path
            ...
            if (currentPath.length === 0) {
                const targetChar = currentSwitchState ? 'W' : (gameState.keyCollected ? 'D' : 'K');
...
console.log(`🚀 Montezuma Advanced TS Agent Started. Polling ${API_URL} as fast as possible...`);
